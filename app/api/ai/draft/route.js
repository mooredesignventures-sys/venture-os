...
const ALLOWED_SCOPES = new Set(["brainstorm"]);
...
function buildBrainstormAssistantText(prompt, bundle, source) {
  ...
  const sampleTitles = nodes
    ...
    .filter(Boolean)
    .slice(0, 3);
  const sampleText = sampleTitles.length ? ` Top ideas: ${sampleTitles.join(" | ")}` : "";
  return `AI ${source === "ai" ? "live" : "fallback"} drafted ${nodeCount} proposed items for "${promptSnippet(prompt)}".${sampleText}`;
}

function toBrainstormPayload(prompt, source, bundle, fallbackReason = "") {
  const proposedNodes = Array.isArray(bundle?.nodes) ? bundle.nodes : [];
  const proposedEdges = Array.isArray(bundle?.edges) ? bundle.edges : [];
  return {
    ok: true,
    source,
    fallbackReason,
    assistantText: buildBrainstormAssistantText(prompt, bundle, source),
    proposedNodes,
    proposedEdges,
  };
}
...
export async function POST(request) {
  ...
  if (process.env.VOS_FORCE_MOCK === "true") {
    ...
    const payload =
      scope === "brainstorm"
        ? toBrainstormPayload(prompt, "mock", bundle, "forced_mock")
        : { ok: true, source: "mock", fallbackReason: "forced_mock", bundle };
  ...
  if (!process.env.OPENAI_API_KEY) {
    ...
    const payload =
      scope === "brainstorm"
        ? toBrainstormPayload(prompt, "mock", bundle, "missing_api_key")
        : { ok: true, source: "mock", fallbackReason: "missing_api_key", bundle };
    return withAiHeaders(
      payload,
      "mock",
    );
  ...
  try {
    ...
    if (!aiBundle) {
      ...
      const payload =
        scope === "brainstorm"
          ? toBrainstormPayload(prompt, "mock", bundle, "openai_response_invalid")
          : {
              ok: true,
              source: "mock",
    ...
    const payload =
      scope === "brainstorm"
        ? toBrainstormPayload(prompt, "ai", sanitized)
        : { ok: true, source: "ai", bundle: sanitized };
    return withAiHeaders(
      payload,
      "ai",
    );
    ...
    const payload =
      scope === "brainstorm"
        ? toBrainstormPayload(prompt, "mock", bundle, fallbackReason)
        : { ok: true, source: "mock", fallbackReason, bundle };
    return withAiHeaders(
      payload,
      "mock",
      fallbackReason,
